...
function buildCharts(_sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var resultArray = bellyButtonSamples.filter(sample => sample.id == _sample);

    // 1c. Create a variable that filters the metadata array for the object with the desired sample number.

    var metaArray = data.metadata.filter(sample => sample.id == _sample);
    ...
    var barLayout = {
      title: "Top 10 Belly Button Bacteria Found",
      margin: { t: 30, l: 150 }
